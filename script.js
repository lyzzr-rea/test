...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let snoozeTaskId = null; // Untuk menyimpan ID task yang akan ditunda
  ...
  function startTimer(task) {
    ...
    if (delay <= 0) {
      // Jika waktu sudah lewat, langsung tampilkan notifikasi
      showNotification(task);
      return;
    }
    // Simpan timeoutId agar bisa dibatalkan nanti
    ...
    if (Notification.permission === "granted") {
      navigator.serviceWorker.ready.then(reg => {
        reg.showNotification("Task Reminder", {
          ...
          icon: "icon-192.png",
          badge: "icon-192.png",
          tag: task.id, // untuk mengelompokkan notifikasi
          data: { taskId: task.id }
        });
      });
    } else {
  ...
  // Menangani klik notifikasi (dari service worker) untuk membuka snooze
